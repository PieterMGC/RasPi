...
I2C_BUS = 1
MOTOR_ADDR = 0x0F   # jouw gevonden adres
...
def motor_set(dir1, dir2, spd1, spd2):
    """
    dir: 0=brake, 1=forward, 2=backward
    spd: 0..255
    """
    spd1 = max(0, min(255, int(spd1)))
    spd2 = max(0, min(255, int(spd2)))

    # eerst richting, dan snelheid
    bus.write_i2c_block_data(MOTOR_ADDR, MOTOR_DIR_REG,   [dir1, dir2])
    bus.write_i2c_block_data(MOTOR_ADDR, MOTOR_SPEED_REG, [spd1, spd2])


def motor_stop():
    # speed 0 is meestal genoeg; richting brake mag ook
...
def apply_motor(thr_norm):
    ...
    if thr_norm > 0:
        # motors vooruit
        motor_set(1, 1, speed_255, 0)
    else:
        # motors achteruit
        motor_set(2, 2, speed_255, 0)
